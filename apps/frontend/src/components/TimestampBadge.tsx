...
export const TimestampBadge = ({
  ...
  if (isFresh) {
    const label = freshLabel ?? (locale === 'ko' ? '최신 분석' : 'Fresh Analysis');
    return (
      <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/20 backdrop-blur-sm ${className}`}>
        ...
        <span className="text-xs font-semibold text-cyan-600 dark:text-cyan-400">{label}</span>
      </div>
    );
  }
  ...
  const prefix = locale === 'ko' ? '업데이트: ' : 'Updated ';
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/20 backdrop-blur-sm ${className}`}>
      ...
      >
        {prefix}{formatRelativeTime(dateObj, locale)}
      </span>
    </div>
  );
};
